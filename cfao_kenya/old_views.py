...
def reset_all_password(request):
    ...
    for staff_u in staffs:
        ...
        link = format_html(
            str('<a href="https://ck-pms.com/accounts/reset/' + encoded_uid + '/' + token + '">PMS Link</a>'))
        pms_link = format_html(str('<a href="https://ck-pms.com/">Online PMS</a>'))

        msg = format_html(
            'We are glad to have you onboard ' + pms_link + '<br><br>Your username: <b>' + user.username + '</b><br>Click on this ' + link + ' to reset your password<br>')
...
class KpiResultUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            reverse('cfao_kenya:Individual_Kpi_Result_Update', kwargs={"pk": self.kwargs["pk"]}))
        ...
        if user_team is not None:
            ...
            if team_leader:
                ...
                for tl in team_leader:
                    send_email_pms('CheckIn Confirmation', User.objects.get(pk=tl.staff_person.id), self.request.user,
                                   e_message)
...
class AssessmentSTlStaff(CreateView):
    ...
    def get_success_url(self):
        return '{}'.format(
            reverse('cfao_kenya:Assessment_TL_One',
                    kwargs={"as_id": self.kwargs["as_id"], "tl_id": self.kwargs["tl_id"]}))
...
class AdminUserNewDetails(UpdateView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            reverse('cfao_kenya:Admin_Users_New_Details_Staff', kwargs={'pk': self.kwargs['pk']}))
...
class AdminPMSBUStaffNew(CreateView):
    ...
    def get_success_url(self):
        return '{}'.format(
            reverse('cfao_kenya:Admin_PMS_BU_Staff',
                    kwargs={"pms_id": self.kwargs["pms_id"], 's_id': self.kwargs['s_id']}))
...
class AdminPMSCompanyOne(UpdateView):
    ...
    def get_success_url(self):
        return '{}'.format(
            reverse('cfao_kenya:Admin_PMS_Company_One',
                    kwargs={"pms_id": self.kwargs["pms_id"], 'kpi_id': self.kwargs['kpi_id']}))
...
class AdminPMSCheckInStaffNew(CreateView):
    ...
    def get_success_url(self):
        return '{}'.format(
            reverse('cfao_kenya:Admin_PMS_CheckIn_Staff',
                    kwargs={"pms_id": self.kwargs["pms_id"], 's_id': self.kwargs['s_id']}))
...
class AdminPMSAssessmentNew(CreateView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            reverse('cfao_kenya:Admin_PMS_Assessment', kwargs={"pms_id": self.kwargs["pms_id"]}))
...
class AdminPMSMatrixScore(UpdateView):
    ...
    def get_success_url(self):
        return '{}'.format(
            reverse('cfao_kenya:Admin_PMS_Matrix_Score',
                    kwargs={"pms_id": self.kwargs["pms_id"], 'm_id': self.kwargs['m_id']}))
            ...
            for staff_u in all_staff:
                if staff_u.staff_md == "Yes":
                    ...
                    if staff_u.staff_head_team is not None:
                        count = 0
                        for eval in evals:
                            if done_tl_evaluates_staff.objects.filter(done_evaluation=eval,
                                                                      done_staff=staff_u.staff_person):
                                count = count + 1
                    ...
                    for eval in evals:
                        if done_tl_evaluates_staff.objects.filter(done_evaluation=eval,
                                                                  done_staff=staff_u.staff_person):
                            count = count + 1
                        if done_staff_evaluates_tl.objects.filter(done_evaluation=eval,
                                                                  done_staff=staff_u.staff_person):
                            count1 = count1 + 1
                    if staff_u.staff_head_team is not None:
                        tl_s = str(count) + "/" + str(evals.count())
                    ...
                    kpi = individual_Kpi.objects.filter(individual_kpi_user=staff_u.staff_person,
                                                        individual_kpi_pms=active_pms).count()
                    role = "Staff"
                    checkin = checkIn.objects.filter(checkIn_pms=active_pms, checkIn_staff=staff_u.staff_person).count()
